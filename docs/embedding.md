...
Processing hundreds of gigabytes of text data can be a daunting task, especially when limited to making rated limited batch requests to a remote API. This article explores how Modal can be used to efficiently embed the huggingface Simple English Wikipedia in under 15 minutes, by mounting the data into a Modal volume and mapping the embeddingfunction across multiple GPUs.

The goal is for you to understand how to run large scale batch embedding jobs on Modal and why this is a valuable capability for anyone to improve and iterate on a RAG application that can learn from user feedback. In the real world, not only will you use and embedding model, you'll want to finetune it on your own data to capture the nuances of your users' language. This will allow you to serve more relevant results to your users and improve the overall experience.

In a future post, we will explain how to finetune your own embedding models on your own data, and given the tools we present today to re-embed the source data quickly and efficiently. This will allow you to serve more relevant results to your users and improve the overall experience as

But for now, lets get started with the basics!

## Why Wikipedia?

Our goal is to demonstrate the how to use Modal to embedded large datasets. This will enable companies to enhance their embedding models by fine-tuning them with their own datasets and re-embedding the source data swiftly and effectively.
